...
class TelloSwarm:
    ...
    @staticmethod
    def fromIps(ips: list, in_ports=None, out_ports=None):
        ...
        if in_ports is None:
            default_in_ports = [8890, 8891, 8892, 8893, 8894, 8895]
            in_ports = default_in_ports[:len(ips)]

        if out_ports is None:
            default_out_ports = [8990, 8991, 8992, 8993, 8994, 8995]
            out_ports = default_out_ports[:len(ips)]

        tellos = []
        for ip, inPort, outPort in zip(ips, in_ports, out_ports):
            tellos.append(Tello(host=ip, control_port=in_ports, state_port=out_ports))
